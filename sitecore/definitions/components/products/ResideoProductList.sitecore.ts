...
/**
 * Adds the ResideoProductList component to the disconnected manifest.
 * This function is invoked by convention (*.sitecore.js) when 'jss manifest' is run.
 * @param {Manifest} manifest Manifest instance to add components to
 */
export default function (manifest: Manifest): void {
  manifest.addComponent({
    name: 'ResideoProductList',
    ...
    fields: [
      {
        ...
        source: `dataSource=/sitecore/content/${packageJson.config.appName}/Content/Products`,
      },
      { name: 'localContentList', type: CommonFieldTypes.ContentList },
    ],
    // inherit fields from another template (../templates/Styleguide-Explanatory-Component)
    // inheritance adds fields defined on the base template(s) implicitly to this component
    inherits: ['resideoproduct-template'],
  });
}
